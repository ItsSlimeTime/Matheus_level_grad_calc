...
A calculator for levels, gradients and ramp landings on architecture projects.

**Open it here:** https://itsslimetime.github.io/Matheus_level_grad_calc/
...
1. Open **Chrome** and go to https://itsslimetime.github.io/Matheus_level_grad_calc/
...
5. The **Level Calc** icon appears on your home screen and in your app list. Open it from there from now on.
...
1. Open **Safari** and go to https://itsslimetime.github.io/Matheus_level_grad_calc/
...
| File | Purpose |
| --- | --- |
| `index.html` | The whole calculator (HTML, CSS, JS and embedded logo). |
| `manifest.webmanifest` | App name, colours and icons used when installing. |
| `sw.js` | Service worker that makes the app work offline. |
| `icons/` | App icons (home screen, favicon, Apple touch icon). |

**Updating the calculator:** if you replace `index.html` with a new version, copy across the two blocks marked `<!-- PWA ... -->` (one in `<head>`, one just before `</body>`). Without them the app can no longer be installed and won't work offline.
